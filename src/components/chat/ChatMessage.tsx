  ...
  if (isUser) {
    return (
      <div className="flex flex-col items-end gap-1">
        <div
          className={cn(
            "max-w-[85%] rounded-2xl rounded-br-sm bg-primary px-4 py-2.5 text-sm font-light leading-relaxed text-white",
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="flex max-w-[85%] flex-col gap-1.5">
        <div
          className={cn(
            "rounded-2xl rounded-bl-sm px-4 py-2.5 text-sm font-light leading-relaxed",
            message.error
          ...
          <div className="flex flex-wrap gap-1.5 px-1">
            {message.citations.map((c, i) =>
              c.url ? (
                ...
                <span
                  key={i}
                  className="inline-flex items-center rounded-full border border-border/70 bg-white px-2.5 py-0.5 text-xs font-normal text-muted-foreground"
                >
                  {c.title}
                </span>
              ),
